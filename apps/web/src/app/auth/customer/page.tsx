      ...
      >
        ...
        <div className="p-8 rounded-2xl border border-white/[0.08] bg-white/[0.02] backdrop-blur-sm">
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              <h1 className="text-xl font-bold">
                {mode === "login" ? "Вход для клиентов" : "Регистрация"}
              </h1>
              <p className="text-sm text-white/40">Управляйте заявками и заказами</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-indigo-500 text-white font-semibold hover:shadow-[0_0_30px_rgba(6,182,212,0.25)] transition-all active:scale-[0.98]"
            >
          ...
          <p className="mt-6 text-center text-sm text-white/30">
            {mode === "login" ? (
              <>Нет аккаунта?{" "}
                <button onClick={() => setMode("register")} className="text-cyan-400 hover:text-cyan-300 transition-colors">
                  Зарегистрироваться
                </button>
              </>
            ) : (
              <>Уже есть аккаунт?{" "}
                <button onClick={() => setMode("login")} className="text-cyan-400 hover:text-cyan-300 transition-colors">
                  Войти
                </button>
              </>
            )}
