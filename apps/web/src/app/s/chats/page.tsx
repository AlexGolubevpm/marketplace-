...
import { MessageSquare, Send, Paperclip, ArrowLeft, User } from "lucide-react";
...
export default function CarrierChatsPage() {
    ...
    return (
      <div className="flex flex-col h-[calc(100vh-8rem)]">
        <div className="flex items-center gap-3 pb-4 border-b border-gray-200">
          ...
          <div>
            <p className="font-semibold text-gray-900">{activeConvo.customer_name}</p>
        ...
        <div className="flex-1 overflow-y-auto py-4 space-y-3">
          {messages.length === 0 && (
            <p className="text-center text-gray-400 text-sm py-10">Начните диалог с клиентом</p>
          )}
          {messages.map((msg) => {
            const isMe = msg.sender_role === "carrier";
            return (
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-gray-500">Общение с клиентами по заявкам</p>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-sm text-gray-400 mt-1">Чаты появятся когда вы ответите на заявку оффером</p>
        </div>
      ) : (
              ...
              <div className="flex-1 min-w-0">
                ...
                {c.last_message && (
                  <p className="text-sm text-gray-500 truncate mt-1">
                    {c.last_message.sender_role === "carrier" ? "Вы: " : ""}{c.last_message.text || "Файл"}
                  </p>
                )}
              </div>
            </button>
