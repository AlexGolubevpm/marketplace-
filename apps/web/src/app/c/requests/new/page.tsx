...
const inputClass = "w-full px-4 py-3 rounded-xl bg-white border border-gray-300 text-gray-900 placeholder:text-gray-400 focus:outline-none focus:border-red-300 focus:ring-2 focus:ring-red-100 transition-colors";
    ...
    return (
      <div className="max-w-md mx-auto text-center py-20">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.4 }}>
          ...
          <p className="text-gray-500 mb-2">Мы отправили её подходящим карго-компаниям.</p>
          <p className="text-gray-500 text-sm">Первые предложения обычно приходят в течение нескольких минут.</p>
      ...
      <div className="flex items-center gap-2">
        {[
          ...
          <div key={s.n} className="flex items-center gap-2 flex-1">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium transition-all ${step >= s.n ? "bg-cyan-500 text-white" : "bg-gray-100 text-gray-400"}`}>{s.n}</div>
            <span className={`text-sm hidden sm:block ${step >= s.n ? "text-gray-600" : "text-gray-400"}`}>{s.label}</span>
            {s.n < 3 && <div className={`flex-1 h-px ${step > s.n ? "bg-cyan-500/50" : "bg-gray-100"}`} />}
          </div>
        ))}
      </div>
      ...
      <AnimatePresence mode="wait">
          ...
          <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
            <div className="p-6 rounded-2xl border border-gray-200 bg-white space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-sm text-gray-500 block mb-1">Город назначения *</label>
        ...
        {step === 2 && (
          <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
            <div className="p-6 rounded-2xl border border-gray-200 bg-white space-y-4">
              ...
              <div>
                <label className="text-sm text-gray-500 block mb-1">Описание груза *</label>
            ...
            <div className="flex gap-3">
              <button onClick={() => setStep(1)} className="flex-1 py-3 rounded-xl border border-gray-200 text-gray-500 font-medium hover:bg-gray-50 transition-colors">Назад</button>
        ...
        {step === 3 && (
          <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
            ...
            <div className="flex gap-3">
              <button onClick={() => setStep(2)} className="flex-1 py-3 rounded-xl border border-gray-200 text-gray-500 font-medium hover:bg-gray-50 transition-colors">Назад</button>
