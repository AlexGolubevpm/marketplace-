      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold">
            Офферы <span className="text-gray-500">({offers.length})</span>
          </h2>
        </div>
      ...
      {order && (
        <div className="p-5 rounded-xl border border-gray-200 bg-white">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
            <div><span className="text-gray-500">Цена:</span> <span className="font-medium">${parseFloat(order.price).toLocaleString()}</span></div>
            {order.tracking_number && <div><span className="text-gray-500">Трекинг:</span> <span className="font-mono font-medium">{order.tracking_number}</span></div>}
            <div><span className="text-gray-500">Создан:</span> <span className="font-medium">{new Date(order.created_at).toLocaleDateString("ru-RU")}</span></div>
          </div>
        </div>
      )}
                ...
                <div key={doc.id} className="flex items-center justify-between p-3 rounded-lg bg-white border border-gray-200">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-400">{DOC_TYPE_LABELS[doc.file_type] || doc.file_type} · {doc.uploaded_by_role} · {new Date(doc.created_at).toLocaleDateString("ru-RU")}</p>
                    </div>
                  </div>
