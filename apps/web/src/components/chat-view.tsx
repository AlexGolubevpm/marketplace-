...
interface ChatViewProps {
  /** "customer" or "carrier" — determines whose perspective this is */
  role: "customer" | "carrier";
  /** The user/entity ID to filter conversations */
  userId: string;
  /** Query param name for loading conversations (e.g. "customer_id" or "carrier_id") */
  queryParam: string;
  /** Name of the counterparty shown in the header */
  counterpartyNameKey: "carrier_name" | "customer_name";
  /** Icon for the counterparty avatar */
  CounterpartyIcon: LucideIcon;
  /** Avatar styling */
  ...
  /** Empty state hint */
  emptyHint: string;
  /** Placeholder for the active chat empty state */
...
export function ChatView({
  ...
  const loadConversations = useCallback(async () => {
    try {
      const res = await fetch(`/api/chats?${queryParam}=${userId}`);
      if (res.ok) {
    ...
    return (
      <div className="flex flex-col h-[calc(100vh-8rem)]">
        <div className="flex items-center gap-3 pb-4 border-b border-gray-200">
          ...
          <div className={`w-10 h-10 rounded-full flex items-center justify-center ${avatarClassName}`}>
            <CounterpartyIcon className={`h-5 w-5 ${iconClassName}`} />
          </div>
          <div>
            <p className="font-semibold text-gray-900">{activeConvo[counterpartyNameKey]}</p>
        ...
        <div className="flex-1 overflow-y-auto py-4 space-y-3">
          {messages.length === 0 && (
            <p className="text-center text-gray-400 text-sm py-10">{chatPlaceholder}</p>
          )}
          {messages.map((msg) => {
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {conversations.map((c) => (
            ...
            >
              <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${avatarClassName}`}>
                <CounterpartyIcon className={`h-5 w-5 ${iconClassName}`} />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <p className="font-medium text-gray-900">{c[counterpartyNameKey]}</p>
                  {c.last_message && (
                ...
                {c.last_message && (
                  <p className="text-sm text-gray-500 truncate mt-1">
                    {c.last_message.sender_role === role ? "Вы: " : ""}{c.last_message.text || "Файл"}
                  </p>
                )}
              </div>
            </button>
